...
export function moonLayer() {
  ...
  function build() {
    ...
    // maria / craters
    const spots = [
      [0.42, 0.3, 0.13, 0.18], [0.6, 0.48, 0.1, 0.16], [0.32, 0.56, 0.08, 0.2],
      [0.55, 0.7, 0.12, 0.14], [0.7, 0.28, 0.06, 0.2], [0.45, 0.46, 0.05, 0.22],
    ];
    for (const [fx, fy, fr, a] of spots) {
      g.fillStyle = `rgba(70,80,100,${a})`;
      g.beginPath(); g.arc(S * fx, S * fy, r * fr, 0, TAU); g.fill();
    }
    // night-side shadow (light from upper-left, matching the scene)
    g.fillStyle = "rgba(7,11,21,0.8)";
    g.beginPath(); g.arc(S * 0.86, S * 0.78, r * 1.06, 0, TAU); g.fill();
    g.restore();
    g.strokeStyle = "rgba(235,242,255,0.12)"; g.lineWidth = 1;
    g.beginPath(); g.arc(S / 2, S / 2, r - 0.5, 0, TAU); g.stroke();
  }
  return {
    name: "moon", z: 6,
    ...
    draw(e) {
      ...
      const tilt = -0.16, ca = Math.cos(ang), sa = Math.sin(ang);
      const ex = ca * R * 2.3, ey = sa * R * 0.8;
...
export function cometLayer() {
  let active = false, t0 = 0, dur = 0, p0, cp, p1, nextAt = 0;
  return {
    name: "comet", z: -2.4,
    simulate(e) {
      if (!e.scene.comet) { active = false; return; }
      if (nextAt === 0) nextAt = e.now + 40000 + Math.random() * 30000;
      if (!active && e.now >= nextAt && e.intro >= 1) {
        active = true; t0 = e.now; dur = 22000 + Math.random() * 6000;
        const { W, H } = e;
    ...
    draw(e) {
      ...
      ctx.beginPath(); ctx.moveTo(bx, by); ctx.lineTo(head[0], head[1]); ctx.stroke();
      // coma + core
      drawGlow(ctx, head[0], head[1], 8, env * 0.9);
      ctx.globalAlpha = env; ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(head[0], head[1], 1.8, 0, TAU); ctx.fill();
      ctx.globalAlpha = 1;
      NORMAL(ctx);
    },
  };
...
// ======================================================================
const FIGURES = [
  { pts: [[0, 0.45], [0.22, 0.1], [0.45, 0.38], [0.7, 0], [1, 0.28]],
    lines: [[0, 1], [1, 2], [2, 3], [3, 4]] },                                  // Cassiopeia
  { pts: [[0.2, 0], [0.75, 0.05], [0.36, 0.45], [0.5, 0.5], [0.64, 0.55], [0.15, 1], [0.85, 0.95]],
    lines: [[0, 2], [1, 4], [2, 3], [3, 4], [2, 5], [4, 6]] },                  // Orion
  { pts: [[0.5, 0], [0.5, 0.35], [0.5, 0.75], [0.15, 0.45], [0.85, 0.25], [0.5, 1]],
    lines: [[0, 1], [1, 2], [2, 5], [3, 1], [1, 4]] },                          // Cygnus
];
const ANCHORS = [[0.16, 0.2], [0.78, 0.64], [0.62, 0.12]];
...
export function constellationsLayer() {
  ...
  return {
    ...
    draw(e) {
      ...
      const ox = -e.look.x * 3, oy = e.look.y * 3;
      const lp = clamp(p / 0.25, 0, 1) * fig.lines.length;
      ADD(ctx);
      ctx.strokeStyle = "#cfe0ff"; ctx.lineWidth = 0.9; ctx.lineCap = "round";
      for (let j = 0; j < fig.lines.length; j++) {
        const seg = clamp(lp - j, 0, 1);
        if (seg <= 0) break;
        const [a, b] = fig.lines[j];
        const [ax, ay] = fig.pts[a], [bx, by] = fig.pts[b];
      ...
      for (let j = 0; j < fig.pts.length; j++) {
        ...
        ctx.globalAlpha = env * tw; ctx.fillStyle = "#eaf2ff";
        ctx.beginPath(); ctx.arc(x + ox, y + oy, 1.1, 0, TAU); ctx.fill();
      }
      ctx.globalAlpha = 1;
      NORMAL(ctx);
    },
...
export function sunGlintLayer() {
  ...
  return {
    ...
    draw(e) {
      ...
      if (a <= 0.01) return;
      const px = CX + R * s.x, py = CY - R * s.y;
      ADD(ctx);
      ctx.save();
      ctx.translate(px, py); ctx.scale(1.25, 0.85);
      ctx.globalAlpha = a;
      const r1 = R * 0.42;
...
// ======================================================================
const HB_PERIOD = 28000, HB_DUR = 2800, HB_MAX = 100 * DEG;
const HB_N = 96;
const HB_COS = new Float64Array(HB_N + 1), HB_SIN = new Float64Array(HB_N + 1);
for (let i = 0; i <= HB_N; i++) { const a = (i / HB_N) * TAU; HB_COS[i] = Math.cos(a); HB_SIN[i] = Math.sin(a); }
export function heartbeatLayer() {
  let t0 = -1, nextAt = 0, hqTrig = null;
  return {
    name: "heartbeat", z: 58,
    build(e) {
      const [lng, lat] = e.data.HQ.lnglat;
      const latR = lat * DEG, lngR = lng * DEG;
      hqTrig = { sinLat: Math.sin(latR), cosLat: Math.cos(latR), sinLng: Math.sin(lngR), cosLng: Math.cos(lngR) };
    },
    visible: (e) => e.scene.heartbeat,
    simulate(e) {
      if (nextAt === 0) nextAt = e.now + 12000;
      if (t0 < 0 && e.now >= nextAt && e.hqVisible && e.intro >= 1 && !e.sim.paused) {
        t0 = e.now; nextAt = e.now + HB_PERIOD;
      }
    ...
    draw(e) {
      ...
      let ux = hz, uy = 0, uz = -hx;
      const ul = Math.hypot(ux, uz);
      if (ul < 1e-4) { ux = 1; uy = 0; uz = 0; } else { ux /= ul; uz /= ul; }
      ...
      for (let i = 0; i <= HB_N; i++) {
        const ct = HB_COS[i], st = HB_SIN[i];
        const qz = hz * cD + (uz * ct + vz * st) * sD;
        if (qz <= 0.02) { pen = false; continue; }
        const qx = hx * cD + (ux * ct + vx * st) * sD;
        const qy = hy * cD + (uy * ct + vy * st) * sD;
        const sxp = CX + R * qx, syp = CY - R * qy;
        if (!pen) { path.moveTo(sxp, syp); pen = true; }
        else path.lineTo(sxp, syp);
      }
      ctx.strokeStyle = rgba("#5ad1ff", a * 0.22); ctx.lineWidth = 5; ctx.stroke(path);
      ctx.strokeStyle = rgba("#8cdfff", a * 0.75); ctx.lineWidth = 1.8; ctx.stroke(path);
  ...
  return {
    ...
    draw(e) {
      ...
      for (const off of [0, 550]) {
        ...
        ctx.strokeStyle = "#7cd4ff"; ctx.lineWidth = 1.2;
        ctx.beginPath(); ctx.arc(x, y, 3 + p * 22, 0, TAU); ctx.stroke();
      }
      ctx.globalAlpha = 1;
      ADD(ctx); drawGlow(ctx, x, y, 10, 0.6 * a); NORMAL(ctx);
      ctx.globalAlpha = a;
      ctx.fillStyle = "#fff";
      ...
      ctx.strokeStyle = "rgba(5,6,12,0.8)"; ctx.lineWidth = 3.5;
      ctx.strokeText(s.city.name, x + 10, y - 8);
      ctx.fillStyle = "#bfe9ff"; ctx.fillText(s.city.name, x + 10, y - 8);
      ctx.globalAlpha = 1;
    },
  };
}
