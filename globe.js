...
(function () {
  ...
  try {
    ...
    Object.keys(saved).forEach(function (k) { if (k in scene) scene[k] = saved[k]; });
  } catch (e) {}
  function saveScene() {
    try { localStorage.setItem("gd-globe-scene", JSON.stringify(scene)); } catch (e) {}
  }
  var DENSITY_STEP = { sparse: 3.8, med: 3.0, dense: 2.4 };
  var AURORA_SCHEMES = {
    gv: ["#5cffb0", "#b58cff"], emerald: ["#3dffa0", "#7affd1"], rose: ["#ff7ab0", "#b58cff"]
  };
  ...
  function layout() {
    ...
    sphereEl.setAttribute("cx", CX); sphereEl.setAttribute("cy", CY); sphereEl.setAttribute("r", R);
    sphereGlow.setAttribute("cx", CX); sphereGlow.setAttribute("cy", CY); sphereGlow.setAttribute("r", R * 1.06);
    highlightEl.setAttribute("cx", CX - R * 0.32); highlightEl.setAttribute("cy", CY - R * 0.34);
    highlightEl.setAttribute("r", R * 1.0);
    bottomGlow.setAttribute("cx", CX); bottomGlow.setAttribute("cy", CY + R * 0.86);
    bottomGlow.setAttribute("rx", R * 0.62); bottomGlow.setAttribute("ry", R * 0.26);
    clipCircle.setAttribute("cx", CX); clipCircle.setAttribute("cy", CY); clipCircle.setAttribute("r", R + 2);
  }
  ...
  var DEG = Math.PI / 180;
  var P_lon0 = 0, P_sinLat0 = 0, P_cosLat0 = 1;
  ...
  var sun_lon = 0, sun_sinLat = 0, sun_cosLat = 1;
  ...
  function renderNight() {
    if (!scene.dayNight) { nightEl.setAttribute("d", ""); nightCoreEl.setAttribute("d", ""); return; }
  ...
  function auroraBand(baseLat, amp, phase, now) {
    ...
    for (var lng = -180; lng <= 180; lng += 4) {
      ...
      var dlon = lngR - P_lon0, cd = Math.cos(dlon);
  ...
  function renderAurora(now) {
    ...
    var t = now * 0.0011, k = scene.auroraIntensity;
    ...
    aurSV.style.opacity = Math.min(1, (0.30 + 0.22 * Math.sin(t + 4.6)) * k).toFixed(2);
  }

  // ---- live ticker --------------------------------------------------------
  var VERBS = {
    trophy: "earned a Trophy", platinum: "unlocked a Platinum",
    newgame: "started a new game", friend: "made a new friend",
    levelup: "leveled up", completed: "completed a game"
  };
  var lastTick = -1e9;
  function pushTicker(type, cityName) {
    ...
    line.innerHTML =
      '<span class="tk-dot" style="color:' + col + ';background:' + col + '"></span>' +
  ...
  // ---- build dotted land --------------------------------------------------
  var dotSin, dotCos, dotLng, dotCity, dotGrp, dotTier, dotN = 0;
  function buildLandDots() {
    var step = DENSITY_STEP[scene.density] || 3.0;
    var sinA = [], cosA = [], lngA = [], cityA = [], grpA = [], tierA = [], pts = [];
    for (var lat = -84; lat <= 84; lat += step) {
      ...
      for (var lng = -180; lng < 180; lng += ringStep) {
        if (d3.geoContains(landFeature, [lng, lat])) {
          var latR = lat * DEG;
          sinA.push(Math.sin(latR)); cosA.push(Math.cos(latR)); lngA.push(lng * DEG);
          pts.push([lng, lat]);
  ...
  function renderDots(now) {
    var ds = "", dm = "", dl = "", c0 = "", c1 = "", c2 = "";
    var dayN = scene.dayNight, cityOn = scene.cityLights;
    for (var i = 0; i < dotN; i++) {
      ...
      var tier = dotTier[i];
      if (tier === 0) ds += seg; else if (tier === 1) dm += seg; else dl += seg;
      ...
      if (cityOn && dayN && dotCity[i]) {
        ...
        if (cosSun < 0.04) {
          var g = dotGrp[i];
          if (g === 0) c0 += seg; else if (g === 1) c1 += seg; else c2 += seg;
        }
      }
    }
    landS.setAttribute("d", ds); landM.setAttribute("d", dm); landL.setAttribute("d", dl);
    city0.setAttribute("d", c0); city1.setAttribute("d", c1); city2.setAttribute("d", c2);
    // gentle staggered twinkle, scaled by configured brightness
    var t = now * 0.0017, cb = scene.cityBright;
  ...
  function buildSpikes() {
    ...
    spkSin = Float64Array.from(sinA); spkCos = Float64Array.from(cosA);
    spkLng = Float64Array.from(lngA); spkLenF = Float64Array.from(lenA);
    spkPhase = Float64Array.from(phA); spkN = sinA.length;
  }
  function renderSpikes(now) {
    ...
    for (var i = 0; i < spkN; i++) {
      ...
      var ex = CX + (px - CX) * (1 + len), ey = CY + (py - CY) * (1 + len);
  ...
  function buildNodes() {
    ...
    for (var i = 0; i < 18; i++) {
      ...
      nodes.push({ ll: ll, el: el2, phase: Math.random() * Math.PI * 2, sp: 1.5 + Math.random() * 2.5, size: 1.3 + Math.random() * 1.6 });
    }
  }
  ...
  function buildOrbits() {
    ...
    defs.forEach(function (cfg) {
      ...
      if (sat) { sat.style.filter = "drop-shadow(0 0 5px #bcd9ff)"; orbitsFront.appendChild(sat); }
  ...
  function renderOrbits(now) {
    ...
    for (var o = 0; o < orbits.length; o++) {
      var ob = orbits[o], cfg = ob.cfg;
      var Rr = R * cfg.rf;
      var ci = Math.cos(cfg.incl), si = Math.sin(cfg.incl);
      var yaw = cfg.yaw0 + now * 0.001 * cfg.spin * 6;
      var cy_ = Math.cos(yaw), sy = Math.sin(yaw);
      var fd = "", bd = "", fStart = true, bStart = true;
      var satIdx = (now * 0.0004 * (o + 1)) % 1 * N | 0;
      for (var k = 0; k <= N; k++) {
        var a = (k / N) * Math.PI * 2;
        var x0 = Math.cos(a), y0 = Math.sin(a), z0 = 0;
        // tilt around X
        var y1 = y0 * ci - z0 * si, z1 = y0 * si + z0 * ci, x1 = x0;
        // spin around Y
        var x2 = x1 * cy_ + z1 * sy, z2 = -x1 * sy + z1 * cy_, y2 = y1;
        var sx = CX + x2 * Rr, sySc = CY - y2 * Rr;
  ...
  function buildHQ() {
    ...
    hqLabel.appendChild(lt); hqLabel.appendChild(sub);
    hqLayer.appendChild(hqRing2); hqLayer.appendChild(hqRing);
    hqLayer.appendChild(hqDot); hqLayer.appendChild(hqLabel);
  }
  function renderHQ() {
    var p = projection(GD.HQ.lnglat);
    var vis = !!p && visible(GD.HQ.lnglat);
    var op = vis ? 1 : 0;
    [hqRing, hqRing2, hqDot, hqLabel].forEach(function (n) { n.style.opacity = op; });
    if (!vis) return;
    hqRing.setAttribute("cx", p[0]); hqRing.setAttribute("cy", p[1]);
    hqRing2.setAttribute("cx", p[0]); hqRing2.setAttribute("cy", p[1]);
    hqDot.setAttribute("cx", p[0]); hqDot.setAttribute("cy", p[1]);
  ...
  function spawnBeam() {
    ...
    var enabled = GD.ACTIVITY_TYPES.filter(function (t) { return state.types[t.id].enabled; });
    if (!enabled.length) return;

    // weighted type pick
    var total = enabled.reduce(function (s, t) { return s + (state.types[t.id].weight || 1); }, 0);
    var r = Math.random() * total, type = enabled[0];
    for (var i = 0; i < enabled.length; i++) { r -= state.types[enabled[i].id].weight || 1; if (r <= 0) { type = enabled[i]; break; } }

    // pick a visible source city (try a handful)
    var city = null;
    for (var k = 0; k < 8; k++) {
      var c = GD.CITIES[(Math.random() * GD.CITIES.length) | 0];
      if (visible(c.lnglat)) { city = c; break; }
    }
    if (!city) return;
    ...
    g.appendChild(glow); g.appendChild(core); g.appendChild(hot); g.appendChild(spark); g.appendChild(head);
    ...
    beams.push({
      type: type, src: city.lnglat, color: col,
      g: g, glow: glow, core: core, hot: hot, spark: spark, head: head,
      t0: performance.now(), impacted: false
    });
  }
  ...
  function renderBeams(now) {
    ...
    for (var i = beams.length - 1; i >= 0; i--) {
      var b = beams[i];
      var age = now - b.t0;
      if (age > LIFE_MS) { removeBeam(b); beams.splice(i, 1); continue; }
      ...
      var t = drawP < 0.5 ? 4 * drawP * drawP * drawP
                          : 1 - Math.pow(-2 * drawP + 2, 3) / 2;
      ...
      if (t >= 1) {
        ...
        var ax = lerp(sp[0], C[0], t), ay = lerp(sp[1], C[1], t);
        var bx = lerp(C[0], hqp[0], t), by = lerp(C[1], hqp[1], t);
      ...
      if (scene.beamTrails && t < 1) {
        var u0 = Math.max(0, t - 0.17), sd = "";
        for (var s = 0; s <= 6; s++) {
          var u = u0 + (t - u0) * (s / 6), iu = 1 - u;
      ...
      if (t < 1) {
        b.head.setAttribute("cx", hx.toFixed(1)); b.head.setAttribute("cy", hy.toFixed(1));
  ...
  function spawnImpact(p, color) {
    ...
    (function tick() {
      ...
      if (a >= 1) { c.remove(); burst.remove(); return; }
  ...
  function spawnFireworkBarrage(p, color) {
    ...
    setTimeout(function () { spawnBurst(p[0] + (Math.random() * 2 - 1) * R * 0.20, p[1] - R * 0.14 * Math.random() - R * 0.04, color, 0.72); }, 210);
    setTimeout(function () { spawnBurst(p[0] + (Math.random() * 2 - 1) * R * 0.24, p[1] - R * 0.02, tint(color), 0.66); }, 410);
  }
  ...
  function updateFireworks(dt) {
    ...
    for (var i = fireParticles.length - 1; i >= 0; i--) {
      var p = fireParticles[i]; p.t += dt;
      if (p.flash) {
        var a = p.t / p.ttl;
        if (a >= 1) { p.el.remove(); fireParticles.splice(i, 1); continue; }
        p.el.setAttribute("r", (3 + a * p.grow).toFixed(1));
        p.el.style.opacity = (1 - a);
        continue;
      }
      if (p.t >= p.ttl) { p.el.remove(); fireParticles.splice(i, 1); continue; }
  ...
  var meteors = [], meteorAcc = 0, meteorSeq = 0;
  function spawnMeteor() {
    // start somewhere along the top or right edge, travel down-left
    var fromRight = Math.random() < 0.45;
    var x, y;
    if (fromRight) { x = W + 40; y = Math.random() * H * 0.6; }
    else { x = Math.random() * W * 0.9; y = -40; }
    var ang = (Math.random() * 0.5 + 0.62) * Math.PI; // ~112°–203°: down & left
    var speed = (W + H) * (0.34 + Math.random() * 0.30); // px/sec
    var vx = Math.cos(ang) * speed, vy = Math.abs(Math.sin(ang)) * speed;
    ...
    var g = el("g", {});
    var trail = el("line", { "class": "met-trail", stroke: "url(#" + id + ")", "stroke-width": (1.3 + Math.random() * 1.1).toFixed(1) });
  ...
  function updateMeteors(dt) {
    if (scene.shootingStars) {
      ...
      var guard = 0;
      while (meteorAcc >= 1 && guard < 3) { spawnMeteor(); meteorAcc -= 1; guard++; }
    } else { meteorAcc = 0; }
    for (var i = meteors.length - 1; i >= 0; i--) {
      var m = meteors[i]; m.t += dt;
      ...
      m.trail.setAttribute("x1", tx.toFixed(1)); m.trail.setAttribute("y1", ty.toFixed(1));
      m.trail.setAttribute("x2", m.x.toFixed(1)); m.trail.setAttribute("y2", m.y.toFixed(1));
  ...
  function frame(now) {
    ...
    if (!state.paused) {
      ...
      var guard = 0;
      while (spawnAcc >= 1 && guard < 12) { spawnBeam(); spawnAcc -= 1; guard++; }
    } else {
      projection.rotate(rotation);
      updateFastProj();
    }
    ...
    if (scene.atmosPulse) {
      ...
      bottomGlow.style.opacity = (scene.atmos * (0.88 + 0.12 * Math.sin(now * 0.0011 + 1.2))).toFixed(3);
    }
  ...
  function onUp() { dragging = false; svg.classList.remove("grabbing"); }
  function pointer(e) {
  ...
  function applyScene() {
    var base = scene.dotSize, tex = scene.texture;
    ...
    aurNG.style.stroke = sch[0]; aurSG.style.stroke = sch[0];
    aurNV.style.stroke = sch[1]; aurSV.style.stroke = sch[1];
  ...
  function buildScene() {
    ...
    var SPEC = [
      { sec: "Texture" },
      { k: "dotSize", t: "slider", l: "Dot size", min: 1.5, max: 4.5, step: 0.1, fmt: function (v) { return v.toFixed(1) + "px"; } },
      { k: "texture", t: "slider", l: "Relief texture", min: 0, max: 0.6, step: 0.02, fmt: function (v) { return Math.round(v / 0.6 * 100) + "%"; } },
      ...
      { sec: "Atmosphere" },
      { k: "atmos", t: "slider", l: "Atmospheric glow", min: 0, max: 2, step: 0.1, fmt: function (v) { return Math.round(v * 50) + "%"; } },
      { sec: "Day & night" },
      { k: "dayNight", t: "toggle", l: "Day / night shadow" },
      { k: "darkness", t: "slider", l: "Night darkness", min: 0, max: 0.9, step: 0.05, fmt: function (v) { return Math.round(v / 0.9 * 100) + "%"; } },
      { k: "cityLights", t: "toggle", l: "City lights" },
      { k: "cityBright", t: "slider", l: "City brightness", min: 0.3, max: 1.4, step: 0.05, fmt: function (v) { return Math.round(v / 1.4 * 100) + "%"; } },
      { sec: "Aurora" },
      { k: "aurora", t: "toggle", l: "Aurora" },
      { k: "auroraIntensity", t: "slider", l: "Intensity", min: 0, max: 1.5, step: 0.05, fmt: function (v) { return Math.round(v / 1.5 * 100) + "%"; } },
      { k: "auroraLat", t: "slider", l: "Latitude", min: 55, max: 82, step: 1, fmt: function (v) { return v + "\u00b0"; } },
      { k: "auroraSpeed", t: "slider", l: "Speed", min: 0, max: 3, step: 0.1, fmt: function (v) { return v.toFixed(1) + "\u00d7"; } },
      { k: "auroraScheme", t: "seg", l: "Colour", opts: [["gv", "Green\u00b7Violet"], ["emerald", "Emerald"], ["rose", "Rose"]] },
      { sec: "Effects" },
      { k: "corona", t: "toggle", l: "Edge corona" },
      { k: "coronaIntensity", t: "slider", l: "Corona intensity", min: 0, max: 0.4, step: 0.02, fmt: function (v) { return Math.round(v / 0.4 * 100) + "%"; } },
      { k: "nodes", t: "toggle", l: "Star nodes" },
      { k: "orbits", t: "toggle", l: "Orbital rings" },
      { sec: "Cosmos" },
      { k: "shootingStars", t: "toggle", l: "Shooting stars" },
      { k: "meteorRate", t: "slider", l: "Meteor frequency", min: 0, max: 1, step: 0.05, fmt: function (v) { return Math.round(v * 100) + "%"; } },
    ...
    var byKey = {};
    SPEC.forEach(function (c) { if (c.k) byKey[c.k] = c; });

    function ctrlHTML(c) {
      if (c.sec) return '<div class="sec-h">' + c.sec + "</div>";
      if (c.t === "slider") {
        return '<div class="sc-slider"><div class="sc-top"><span class="sc-lbl">' + c.l +
          '</span><span class="sc-val" data-val="' + c.k + '"></span></div>' +
          '<input type="range" data-k="' + c.k + '" min="' + c.min + '" max="' + c.max + '" step="' + c.step + '"></div>';
      }
      if (c.t === "toggle") {
        return '<div class="sc-row"><span class="sc-lbl">' + c.l +
          '</span><button class="sw" data-tog="' + c.k + '" aria-pressed="false"></button></div>';
      }
      if (c.t === "seg") {
        var b = c.opts.map(function (o) { return '<button data-seg="' + c.k + '" data-v="' + o[0] + '">' + o[1] + "</button>"; }).join("");
    ...
    host.addEventListener("click", function (e) {
      ...
      if (tog) {
        ...
        applyScene(); saveScene(); return;
      }
      var seg = e.target.closest("[data-seg]");
      if (seg) {
        var sk = seg.getAttribute("data-seg"), sv = seg.getAttribute("data-v");
        scene[sk] = sv;
        host.querySelectorAll('[data-seg="' + sk + '"]').forEach(function (bn) { bn.classList.toggle("on", bn === seg); });
        if (sk === "density") { buildLandDots(); renderDots(performance.now()); }
        applyScene(); saveScene(); return;
      }
    });
    ...
    function setOpen(open) {
      if (open) host.removeAttribute("hidden"); else host.setAttribute("hidden", "");
      gear.setAttribute("aria-pressed", open ? "true" : "false");
      try { localStorage.setItem("gd-globe-scene-open", open ? "1" : "0"); } catch (e) {}
    }
    gear.addEventListener("click", function () {
      setOpen(host.hasAttribute("hidden"));
    });
    var wasOpen = false;
    try { wasOpen = localStorage.getItem("gd-globe-scene-open") === "1"; } catch (e) {}
  ...
  function buildControls() {
    ...
    GD.ACTIVITY_TYPES.forEach(function (t) {
      ...
      row.innerHTML =
        '<label class="swatch" style="--c:' + t.color + '">' +
          '<input type="color" value="' + t.color + '" data-color="' + t.id + '">' +
        '</label>' +
    ...
    rate.value = state.rate; rateVal.textContent = state.rate.toFixed(1) + "/s";
    ...
    rot.value = state.rotSpeed; rotVal.textContent = state.rotSpeed + "°/s";
    ...
    fwSel.addEventListener("change", function () { state.fwTrigger = fwSel.value; });
  ...
  function loadLand(i) {
    i = i || 0;
    if (i >= LAND_URLS.length) { fail("all map sources unreachable"); return; }
    fetch(LAND_URLS[i])
