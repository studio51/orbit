...
export function meteorsLayer() {
  let layerEl, meteors = [], acc = 0, seq = 0;
  return {
    name: "meteors", z: 0,
    build(e) {
      layerEl = $("shooting");
  ...
  function spawn(e) {
    ...
    const g = el("g", {});
    const trail = el("line", { "class": "met-trail", stroke: "url(#" + id + ")", "stroke-width": (1.3 + Math.random() * 1.1).toFixed(1) });
...
export function atmosphereLayer() {
  ...
  return {
    ...
    build(e) { sphereGlow = $("sphere-glow"); bottomGlow = $("bottom-glow"); },
    resize(e) {
      const { CX, CY, R } = e;
...
export function sphereLayer() {
  ...
  return {
    name: "sphere", z: 30,
    build(e) { sphereEl = $("sphere"); highlightEl = $("sphere-highlight"); clipCircle = $("clip-circle"); },
    resize(e) {
      const { CX, CY, R } = e;
    ...
    draw(e) { /* static — positioned on resize */ },
  };
}
...
function makeOrbitState() {
  ...
  function build(e) {
    ...
    ORBIT_DEFS.forEach((cfg) => {
      ...
      let sat = null;
      if (cfg.sat) { sat = el("circle", { "class": "orbit-sat", r: 2 }); sat.style.filter = "drop-shadow(0 0 5px #bcd9ff)"; orbitsFront.appendChild(sat); }
...
export function orbitsBackLayer() {
  return {
    ...
    build(e) { orbitState.build(e); },
    draw(e) { orbitState.render(e); }, // computes both front + back path strings
  };
}
export function orbitsFrontLayer() {
  return {
    name: "orbitsFront", z: 80,
    draw(e) { /* path strings already set by orbitsBack's render */ },
  };
}
...
export function spikesLayer() {
  let spikesEl, sin, cos, lng, lenF, phase, n = 0;
  return {
    name: "spikes", z: 35,
    build(e) {
      spikesEl = $("spikes");
    ...
    draw(e) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const ex = CX + (px - CX) * (1 + len), ey = CY + (py - CY) * (1 + len);
...
export function landLayer() {
  ...
  return {
    ...
    build(e) {
      landS = $("land-s"); landM = $("land-m"); landL = $("land-l");
    ...
    draw(e) {
      ...
      const { lon0, sinLat0, cosLat0 } = proj;
      const sun = proj.sun, dayN = scene.dayNight, cityOn = scene.cityLights;
      let ds = "", dm = "", dl = "", c0 = "", c1 = "", c2 = "";
      for (let i = 0; i < n; i++) {
        const dlon = lng[i] - lon0, cd = Math.cos(dlon);
        const cosc = sinLat0 * sin[i] + cosLat0 * cos[i] * cd;
        ...
        const t = tier[i];
        if (t === 0) ds += seg; else if (t === 1) dm += seg; else dl += seg;
      ...
      const t = now * 0.0017, cb = scene.cityBright;
...
export function nightLayer() {
  ...
  return {
    name: "night", z: 55,
    build(e) { nightEl = $("night"); nightCoreEl = $("night-core"); },
    draw(e) {
      if (!e.scene.dayNight) { nightEl.setAttribute("d", ""); nightCoreEl.setAttribute("d", ""); return; }
...
export function cityLightsLayer() {
  return {
    ...
    draw(e) { /* city-light paths are produced by landLayer.draw */ },
  };
}
  ...
  return {
    ...
    draw(e) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (!p || !proj.visible(node.ll)) { node.el.style.opacity = 0; continue; }
...
export function beamsLayer() {
  ...
  return {
    ...
    spawn(e) {
      ...
      beams.push({ type, src: city.lnglat, color: col, g, glow, core, hot, spark, head, t0: e.now, impacted: false });
    },
    draw(e) {
      const { CX, CY, R, now, scene, proj } = e;
      ...
      for (let i = beams.length - 1; i >= 0; i--) {
        ...
        if (t < 1) {
          ...
          if (!b.impacted) {
            ...
            if (e.sim.fireworks && b.type.id === e.sim.fwTrigger) e.emit("fireworks", { p: hqp.slice(), color: b.color });
          }
        }
  ...
  return {
    ...
    build(e) {
      ...
      e.on("impact", (d) => {
        ...
        const burst = el("circle", { "class": "impact-burst", cx: d.p[0], cy: d.p[1], r: 3, fill: d.color });
        layerEl.appendChild(c); layerEl.appendChild(burst);
    ...
    draw(e) {
      ...
      for (let i = items.length - 1; i >= 0; i--) {
        ...
        if (a >= 1) { it.c.remove(); it.burst.remove(); items.splice(i, 1); continue; }
...
export function hqLayer() {
  ...
  return {
    ...
    build(e) {
      ...
      layerEl.appendChild(hqRing2); layerEl.appendChild(hqRing);
      layerEl.appendChild(hqDot); layerEl.appendChild(hqLabel);
    },
    draw(e) {
      const p = e.hq, vis = e.hqVisible;
      const op = vis ? 1 : 0;
      [hqRing, hqRing2, hqDot, hqLabel].forEach((nde) => { nde.style.opacity = op; });
      if (!vis || !p) return;
